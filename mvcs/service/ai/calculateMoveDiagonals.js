var CalculateMoveDiagonals = function(setX, setY) 
{
	...
	this.checkMoveToKill = function(inputMoveArea, unitId, searchStep, setCalculateCpuMove)
	{
		...
		if (!_calculateCpuMove.stopSearch) 
		{
			...
			{
				tempArea = CalculateHighlightDiagonals(inputMoveArea[i].x, inputMoveArea[i].y);
	...
	this.findFirstStep = function(unitMoveMap, steps, target)
	{
		...
		if (steps > firsStepNum)
		{
			for(i=steps; i!=firsStepNum; i--)
			{
				for(j=-10; j<=10; j++)
				{			
					if (this.checkStepId(unitMoveMap,currentposition.y+j,currentposition.x+j,i-1))
					{
						currentposition.x = currentposition.x+j;
						currentposition.y = currentposition.y+j;
						break;
					} else 
					if (this.checkStepId(unitMoveMap,currentposition.y+j*(-1),currentposition.x+j,i-1))
					{
						currentposition.x = currentposition.x+j;
						currentposition.y = currentposition.y+j*(-1);
						break;
					}					
				}
			}
